...
const MagicBento: React.FC<BentoProps> = ({
  ...
  return (
    <>
      ...
      <div
        className="bento-section grid gap-2 p-3 w-full max-w-5xl select-none relative"
        style={{ fontSize: 'clamp(1rem, 0.9rem + 0.5vw, 1.5rem)' }}
        ref={gridRef}
      >
        <div className="card-responsive grid gap-3">
          {cards.map((card, index) => {
            const baseClassName = `card flex flex-col justify-between relative min-h-[220px] w-full max-w-full p-5 rounded-2xl border border-solid font-light overflow-hidden transition-all duration-300 ease-in-out hover:-translate-y-0.5 ${enableBorderGlow ? 'card--border-glow' : ''}`;
            const cardStyle = {
              backgroundColor: card.color ?? '#0d2d3b',
              borderColor: 'rgba(38,119,153,0.25)',
              color: '#ffffff',
              '--glow-x': '50%',
              '--glow-y': '50%',
              '--glow-intensity': '0',
            ...
            const cardContent = (
              <>
                <div className="card__header flex items-center gap-2 relative text-white/80 mb-2">
                  {card.icon && (
                    <div className="flex items-center justify-center w-8 h-8 rounded-lg bg-white/10 border border-white/15 text-white/70 shrink-0">
                      {card.icon}
                    </div>
                  )}
                  <span className="text-xs font-medium text-white/50 uppercase tracking-widest">{card.label}</span>
                </div>

                <div className="card__content flex flex-col relative text-white gap-1">
                  <h3 className={`card__title font-semibold text-base m-0 text-white ${textAutoHide ? 'text-clamp-1' : ''}`}>
                    {card.title}
                  </h3>
                  <p className={`card__description text-xs leading-5 text-white/60 ${textAutoHide ? 'text-clamp-2' : ''}`}>
                    {card.description}
                  </p>
                </div>
                ...
                {card.skills && card.skills.length > 0 && (
                  <div className="flex flex-wrap gap-1.5 mt-3">
                    {card.skills.map(skill => (
                      <span
                        key={skill.name}
                        className="flex items-center gap-1 px-2 py-0.5 rounded-full border border-white/15 bg-white/10 text-[10px] text-white/75"
                      >
