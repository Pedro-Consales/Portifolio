...
function SkillCard({ icon, title, description, skills }: SkillCardProps) {
  return (
    <div className="rounded-2xl border border-white/8 bg-[#EAF0F6] dark:bg-[#1b4c61] p-6 font-mono">
      <div className="flex items-center gap-3 mb-2">
        ...
        <h3 className="text-[17px] font-medium text-[#1A2233] dark:text-slate-100">{title}</h3>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        {skills.map((skill) => (
          <span
            key={skill.name}
            className="flex items-center gap-1.5 px-3 py-1 rounded-full border border-[#CBD5E0] bg-[#FFFFFF] text-[12px] text-slate-300 dark:bg-[#162024] dark:border-[#267799]"
          >
...
export default function SkillOverview() {
  return (
    <section id="skill-overview">
      <main className="flex flex-col justify-center items-center gap-20 p-8">
        <div className="flex flex-col justify-center items-center gap-2">
          <h4 className="font-bold text-6xl">
            {" "}
                <span className="text-[#267799] font-bold ">
                  Skill
                </span>{" "} Overview
          </h4>
          <p>Technologies, tools, and foundations I work with on a daily basis</p>
        </div>
